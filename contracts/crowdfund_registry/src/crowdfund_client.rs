use soroban_sdk::{ IntoVal, Address, BytesN, Env, Symbol, Val, Vec };
...
pub fn deposit(env: &Env, crowdfund_address: Address, donor: Address, amount: i128) {
    ...
    let _: Val = env.invoke_contract(&crowdfund_address, &Symbol::new(env, "deposit"), args);
...
pub fn withdraw(env: &Env, crowdfund_address: Address, drawer: Address) {
    ...
    // Invoke the deposit function on the crowdfund contract.
    let _: Val = env.invoke_contract(&crowdfund_address, &Symbol::new(env, "withdraw"), args);
}
