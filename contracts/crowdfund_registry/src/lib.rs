...
fn set_crowdfund_to_map(env: &Env, crowdfund_id: u64, crowdfund_address: Address) {
    ...
    env.storage()
        .instance()
        .set(&DataKey::CrowdfundMap, &crowdfund_map);
}

fn get_crowdfund_from_map(env: &Env, crowdfund_id: u64) -> Address {
    let crowdfund_map = env
        .storage()
        .instance()
        .get::<_, Map<u64, Address>>(&DataKey::CrowdfundMap)
        .unwrap();
...
fn creat_crowdfund(env: &Env, crowdfund_args: &CrowdfundArgs) -> u64 {
    ...
    let init_args: Vec<Val> = (
        crowdfund_args.recipient.clone(),
        crowdfund_args.deadline.clone(),
        crowdfund_args.target_amount.clone(),
        crowdfund_args.token.clone(),
    )
        .into_val(env);
    let deployed_address = crowdfund_client::deploy(
        &env,
        crowdfund_args.recipient.clone(),
        salt,
...
fn creat_crowdfund_v2(env: &Env, wasm_hash: &BytesN<32>, crowdfund_args: CrowdfundArgs) -> Address {
    ...
    let init_args: Vec<Val> = (
        crowdfund_args.recipient,
        crowdfund_args.deadline,
        crowdfund_args.target_amount,
        crowdfund_args.token,
    )
        .into_val(env);
    let crowdfund_address =
        crowdfund_client::deploy_v2(&env, deployer, wasm_hash.clone(), salt, init_args);
    crowdfund_address
}
...
impl CrowdfundRegistryContract {
    pub fn initialize(env: Env, admin: Address) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::CrowdfundMap, &Map::<u64, Address>::new(&env));
    ...
    pub fn creat_batch_crowdfunds_v2(
        env: Env,
        wasm_hash: BytesN<32>,
        ...
        for crowdfund_arg in crowdfund_args_list.iter() {
            // todo how the crowdfund_args value be moved?
    ...
    pub fn deposit_to_batch_crowdfunds(env: Env, batch_pledge: Vec<Pledge>) {
        ...
        for pledge in batch_pledge.iter() {
            ...
            crowdfund_client::deposit(
                &env,
                crowdfund_address.clone(),
                pledge.donor.clone(),
                pledge.amount.clone(),
            );
        }
